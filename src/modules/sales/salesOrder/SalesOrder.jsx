...
const schema = yup.object().shape({
    ...
    remark: yup.string().trim().required("Please enter remark"),
    products: yup.array().of(
        yup.object().shape({
            product: yup.string().required('Please select a product'),
            quantity: yup.number().positive().required('Please enter a positive number'),
            unitPrice: yup.number().positive().required('Please enter a positive number'),
        })
    ),
}).required();
...
export default function SalesOrder() {
    ...
    useEffect(async () => {
        ...
        if (!isAddMode) {
            ...
            ApiService.get(`salesOrder/${id}`).then(response => {
                setLoderStatus("SUCCESS");
                            ...
                            <Form.Group as={Col} md="4" className="mb-2">
                                ...
                                <Form.Select id="customer" name="customer" {...register("customer", { required: true })} data-live-search="true"
                                    ...
                                    {customerList.map((element, index) => {
                                        ...
                                        return <option value={element.id} key={index} value={element.id}>{element.name}</option>
                                    })}
                                </Form.Select>
                                {errors.customer?.message && <p style={{ color: "red" }}>{errors.customer?.message}</p>}
                            </Form.Group>
                            <Form.Group as={Col} md="4" className="mb-2">
                        ...
                        <Row>
                            <Form.Group as={Col} md="4" className="mb-2">
                                ...
                                <Form.Control
                                    ...
                                    {...register("remark")}
                                />
                                {errors.remark?.message && <p style={{ color: "red" }}>{errors.remark?.message}</p>}
                            </Form.Group>
                        </Row>
                                                        ...
                                                        <tr key={field.id} >
                                                            ...
                                                            <td>
                                                                <Form.Group >
                                                                    <Form.Control
                                                                        ...
                                                                        onBlur={(e) => {
                                                                            console.log(e.target.value);
                                                                            if (e.target.value === "" && getValues(`products.${index}.product`) === "") {
                                                                                setValue(`products.${index}.quantity`, 1)
                                                                                setValue(`products.${index}.taxes`, 0)
                                                                            } else if (String(e.target.value).includes("-")) {
                                                                                setValue(`products.${index}.quantity`, 1)
                                                            ...
                                                            <td>
                                                                <Form.Group>
                                                                    <Form.Control
                                                                        ...
                                                                        onBlur={async (e) => {
                                                                            ...
                                                                            if (e.target.value === "" || getValues(`products.${index}.product`) === "") {
                                                                                ...
                                                                                const v = getValues(`products.${index}.product`);
                                                                                console.log(v)
                                                                                const product = await ApiService.get('product/' + v);
                                                                                setValue(`products.${index}.quantity`, 1)
                                                                                setValue(`products.${index}.unitPrice`, product?.data?.document.salesPrice.toFixed(2))
                                                                                const values = getValues([`products.${index}.unitPrice`, `products.${index}.quantity`]);
                                                                                setValue(`products.${index}.subTotal`, parseFloat(values[0]) * parseInt(values[1]));
                                                                    ...
                                                                    {errors?.['products']?.[index]?.['unitPrice']?.['message'] && <p style={{ color: "red" }}>{errors?.['products']?.[index]?.['unitPrice']?.['message']}</p>}
                                                                </Form.Group>
                                                            </td>
                                                            {/* <td>
                                                                <Form.Group >
